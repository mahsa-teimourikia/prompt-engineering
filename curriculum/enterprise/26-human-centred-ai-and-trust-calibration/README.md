...
An industrial-support answer with high claimed confidence still requires human review when it proposes bypassing a safety control. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](26_human_centred_ai_and_trust_calibration.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![Human-Centred AI and Trust Calibration architecture](diagram-1.svg)
...
Citation interfaces, editable drafts, review queues, and uncertainty signals can help. Token probabilities and self-reported confidence are not generally calibrated factuality measures.
...
The [reusable lab](lab26.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**Self-reported confidence is not permission and can be badly calibrated.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/enterprise/26-human-centred-ai-and-trust-calibration/26_human_centred_ai_and_trust_calibration.ipynb
...
2. Why does the failure case—Self-reported confidence is not permission and can be badly calibrated.—invalidate a happy-path-only evaluation?
