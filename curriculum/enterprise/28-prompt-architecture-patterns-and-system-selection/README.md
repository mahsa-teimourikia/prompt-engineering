...
Northstar selects the least complex architecture that meets auditability, adaptability, latency, and operating-cost needs. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](28_prompt_architecture_patterns_and_system_selection.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![Prompt Architecture Patterns and System Selection architecture](diagram-1.svg)
...
Single calls, retrieval, tools, workflows, and agents form a complexity ladder. Selection is a requirements and evidence exercise, not a popularity contest.
...
The [reusable lab](lab28.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**A single weighted score can disguise fragile assumptions and false precision.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/enterprise/28-prompt-architecture-patterns-and-system-selection/28_prompt_architecture_patterns_and_system_selection.ipynb
...
2. Why does the failure case—A single weighted score can disguise fragile assumptions and false precision.—invalidate a happy-path-only evaluation?
...
- [Berkeley: Compound AI Systems](https://bair.berkeley.edu/blog/2024/02/18/compound-ai-systems/)
