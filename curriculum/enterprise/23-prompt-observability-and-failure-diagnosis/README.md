...
A wrong price answer must be traced to stale evidence without logging customer email addresses or hidden reasoning. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](23_prompt_observability_and_failure_diagnosis.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![Prompt Observability and Failure Diagnosis architecture](diagram-1.svg)
...
Standard traces improve portability. OpenTelemetry's generative-AI conventions are evolving, so pin a version and avoid sensitive content capture by default.
...
The [reusable lab](lab23.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**A complete transcript can leak data while still omitting the version and freshness fields needed for diagnosis.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/enterprise/23-prompt-observability-and-failure-diagnosis/23_prompt_observability_and_failure_diagnosis.ipynb
...
2. Why does the failure case—A complete transcript can leak data while still omitting the version and freshness fields needed for diagnosis.—invalidate a happy-path-only evaluation?
