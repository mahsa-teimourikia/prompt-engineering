...
- Assemble contract, evaluation, threat, trace, ownership, and rollback evidence.
- Run a fail-closed readiness gate.
- Distinguish missing evidence from failed evidence.

## Why this matters

A release board reviews a portfolio of executable evidence rather than accepting a polished demo or checklist claim. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](29_ai_system_engineering_capstone.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![AI System Engineering Capstone architecture](diagram-1.svg)
...
The [reusable lab](lab29.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**A green average score cannot override critical failures, missing ownership, or an untested rollback.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/enterprise/29-ai-system-engineering-capstone/29_ai_system_engineering_capstone.ipynb
...
2. Why does the failure case—A green average score cannot override critical failures, missing ownership, or an untested rollback.—invalidate a happy-path-only evaluation?
