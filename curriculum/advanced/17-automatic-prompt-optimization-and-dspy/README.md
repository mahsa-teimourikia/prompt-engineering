...
A bounded optimizer selects an extraction strategy on development cases, then receives one chance on the sealed holdout. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](17_automatic_prompt_optimization_and_dspy.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![Automatic Prompt Optimization and DSPy architecture](diagram-1.svg)
...
DSPy and related optimizers compile prompt programs against a metric. Bounded search must still be checked for leakage and metric gaming.
...
The [reusable lab](lab17.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**An optimizer can exploit metric shortcuts or memorize exposed answers.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/advanced/17-automatic-prompt-optimization-and-dspy/17_automatic_prompt_optimization_and_dspy.ipynb
...
2. Why does the failure case—An optimizer can exploit metric shortcuts or memorize exposed answers.—invalidate a happy-path-only evaluation?
