...
Northstar compares model profiles behind one typed contract rather than scattering provider-specific behavior across the application. A persuasive demonstration is not sufficient evidence: the system must expose its inputs, decisions, failures, metrics, and release policy.
...
**Success criteria:** the [notebook](20_model_aware_prompt_engineering.ipynb) runs without credentials, its positive and failure assertions pass, and the learner can explain which controls are deterministic and which production behaviors would remain probabilistic.
...
![Model-Aware Prompt Engineering architecture](diagram-1.svg)
...
The [reusable lab](lab20.py) implements the deterministic primitive. The notebook introduces the scenario, runs the baseline and candidate on the same fixture, injects this failure—**A model migration can change output, tool use, latency, safety behavior, and token accounting.**—and finishes with assertions plus a production-upgrade exercise.

Run it from the repository root:

```bash
PYTHONPATH=. .venv/bin/python scripts/run_notebooks.py curriculum/advanced/20-model-aware-prompt-engineering/20_model_aware_prompt_engineering.ipynb
...
2. Why does the failure case—A model migration can change output, tool use, latency, safety behavior, and token accounting.—invalidate a happy-path-only evaluation?
...
- [Deep course guide](../../../docs/16-model-aware-guidance.md)
