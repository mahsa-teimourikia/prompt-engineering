...
A model *proposes* data; the application decides whether it is valid and authorized. Provider-native structured-output features can constrain a response to a documented, supported schema subset, but applications must still handle refusals, truncation, unsupported schema features, transport failures, and version differences. No API can guarantee that schema-valid content is factually true or aligned with business policy; that requires deterministic application-side validation.
...
The pipeline has four increasingly strong checks:
...
| Pattern | Use it when | Contract strength | Typical failure |
| --- | --- | --- | --- |
...
1. **Native Structured Decoding:** Providers such as Google support schema-constrained generation through settings including `response_schema`. This substantially improves shape adherence, but applications must still handle refusals, truncation, provider schema subsets, and validation failures. Prompt-only “JSON mode” is weaker, not universally obsolete.
...
- Demonstration 3 makes exactly two attempts; the second binds the refund
  intent to `pol_return_30d` and passes application-side validation.
...
The [notebook](04_structured_outputs_and_typed_interfaces.ipynb) uses deterministic replays to compare parseable typed output with malformed and semantically invalid output. In an optional live run, the Google GenAI SDK can constrain generation to a supported response schema, but refusals, truncation, unsupported schema features, and application rules still require explicit handling. The same application-side validator and bounded repair path apply in both modes.
