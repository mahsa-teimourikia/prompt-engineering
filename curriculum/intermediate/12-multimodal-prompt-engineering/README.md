...
**Level:** Intermediate · **Estimated time:** 75–120 min · **Prerequisites:** Courses 01–11 and basic image handling

## Learning Objectives

- **Integrate Non-Text Modalities:** Pass images, video, and audio directly into the model context.
- **Understand Multimodal Pipelines:** Compare provider-native media inputs with OCR, transcription, document parsing, and hybrid pipelines.
...
A synthetic invoice image says $500 while users may claim a different amount; extraction and reconciliation must be independently controlled.

## Core Concepts & Workflow

Images, documents, audio, and video can enter a system through provider-native multimodal inputs or through specialist components such as OCR, document parsers, and speech-to-text. Neither architecture is universally superior: native inputs preserve visual or temporal context, while specialist components can provide precise text, coordinates, confidence, lower cost, or easier validation. Multimodal prompt engineering starts by choosing the evidence pipeline, then uses text instructions and source locations to make observations reviewable.
...
See [Prompt security](../../intermediate/13-prompt-security-and-untrusted-content/README.md) for the broader defense-in-depth model. The model’s visual understanding is a probabilistic layer, not an authorization service.
...
Run the credential-free, self-contained [multimodal notebook](../../intermediate/12-multimodal-prompt-engineering/12_multimodal_prompt_engineering.ipynb). Extend it with rotated pages, conflicting totals, low OCR confidence, and injected document text. Keep the sample data synthetic and all consequences mocked.
...
- [Prompt security](../../intermediate/13-prompt-security-and-untrusted-content/README.md) for untrusted content and tool boundaries
- [Prompt evaluation](../../../docs/07-evaluation.md) for datasets, rubrics, and release gates
- [RAG and tool prompting](../../intermediate/10-evidence-grounded-prompting-and-rag-interfaces/README.md) for evidence retrieval and grounding
- [Technology review](../../../docs/10-technology-review.md) for provider, retrieval, and observability decisions
...
1. **Native multimodal APIs:** Current providers accept combinations of text and media, with modality, size, duration, and encoding limits that vary by model and API version. Pin the exact supported contract in production.
2. **Spatial grounding:** Some models can propose coordinates or regions. Coordinate conventions, resize transforms, and localization quality must be validated against labelled examples.
3. **Long-media processing:** Video and audio support can simplify prototyping, but sampling behavior, temporal recall, context limits, privacy, latency, and cost require task-specific evaluation.

## Lab walkthrough

The [notebook](12_multimodal_prompt_engineering.ipynb) passes a checked-in synthetic image through the provider-neutral Northstar interface. The typed extraction includes page, region, and amount-confidence fields; deterministic reconciliation—not the model's contradiction boolean—decides whether the extraction is accepted or routed to review.

The [notebook](12_multimodal_prompt_engineering.ipynb) keeps each experiment visible: it prints the rendered request, recorded response, parsed value, and deterministic assertion. Replay is offline by default; live mode is opt-in.
...
2. Change one prompt, policy, or budget variable in `lab12.py`; inspect the visible request, response, and deterministic gate.
...
- **Cost and Latency:** Images, audio, and video can materially increase input processing, latency, and cost. Measure with the chosen provider and media settings rather than relying on a universal token estimate.
...
- https://aclanthology.org/2026.acl-long.204.pdf
...
- https://www.paddleocr.ai/v2.9.1/en/ppstructure/overview.html
