...
class InvoiceExtraction(BaseModel):
    ...
    source_page: int = Field(ge=1, description="One-based page containing the amount.")
    source_region: str = Field(min_length=1, description="Inspectable region or field locator.")
    amount_confidence: float = Field(
        ge=0,
        le=1,
        description="A model/OCR signal that must be calibrated before production use.",
    )
...
def route(
    ...
    claimed: float | None,
) -> Literal["human_review", "accepted_extraction"]:
    extracted_amount = (
    ...
    if (
        ...
        or extraction.amount_confidence < 0.9
    ):
        return "human_review"
    return "accepted_extraction"
...
def run_lab(client: ModelClient) -> dict[str, Metric | str]:
    ...
    return {
        "routing_accuracy": rate(
            "routing_accuracy",
            sum(decision == case["expected"] for decision, case in zip(decisions, CASES)),
            len(CASES),
            "higher_is_better",
        ),
