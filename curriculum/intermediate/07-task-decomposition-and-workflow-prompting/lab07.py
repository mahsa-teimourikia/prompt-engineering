...
def refund_eligible(
    ...
    *,
    tenant: str,
    today: date = TODAY,
) -> Literal["eligible", "ineligible", "not_found"]:
    order = next(
        (
            item
            for item in ORDERS
            if item["order_id"] == order_id and item["tenant"] == tenant
        ),
        None,
    )
    if order is None:
...
def run_workflow(
    ...
    *,
    ...
    if extracted.order_id == "UNKNOWN":
        ...
        return trace
    eligibility = refund_eligible(extracted.order_id, tenant=tenant)
    trace.steps.append(
...
def run_lab(client: ModelClient) -> dict[str, Metric | list[str]]:
    ...
    for case, trace in zip(CASES, traces):
        ...
        eligibility = refund_eligible(case["expected_order_id"], tenant="tenant-synthetic-a")
    ...
    return {
        ...
        "terminal_accuracy": rate(
            "terminal_accuracy",
            sum(trace.terminal_state == case["expected_terminal"] for trace, case in zip(traces, CASES)),
            len(CASES),
            "higher_is_better",
        ),
    }
