...
- **Validate Citation Contracts:** Require evidence identifiers in the response, then verify each identifier and claim against the authorized retrieved set.
...
The [notebook](10_evidence_grounded_prompting_and_rag_interfaces.ipynb) runs an auditable offline RAG slice. Trusted tenant and approval filters run before ranking, the exact selected evidence is serialized into the generation request, and deterministic checks distinguish an existing citation ID from claim-level support. The recorded no-evidence case abstains, but that is a measured fixture outcome—not proof that prompt wording forces every model to abstain.
