...
**Level:** Intermediate · **Estimated time:** 75–120 min · **Prerequisites:** Courses 01–07 and retrieval basics
...
A customer request contains instruction-like text and a node-count request; the application must assemble only the context and actions allowed by policy.
...
See [RAG and tools](../../intermediate/10-evidence-grounded-prompting-and-rag-interfaces/README.md) for retrieval/tool contracts, [Prompt security](../../intermediate/13-prompt-security-and-untrusted-content/README.md) for boundary controls, and [Cost and latency engineering](../../../docs/13-cost-latency-engineering.md) for operational budgeting.
...
Pair these with answer-level checks: decision correctness, claim-to-source support, correct abstention, and policy-compliant wording. See [Evaluation](../../../docs/07-evaluation.md) for an end-to-end evaluation workflow.

#### Run the course material

- Work through [Notebook 03 — context engineering](../../intermediate/08-context-engineering/08_context_engineering.ipynb), which explains the scenario and includes the deterministic implementation for modifying budgets, freshness, and adversarial fixtures.
- Continue with [RAG and tools](../../intermediate/10-evidence-grounded-prompting-and-rag-interfaces/README.md) to turn this selection policy into retrieval and tool contracts.
...
- [Instruction contracts](../../beginner/02-instruction-contracts/README.md) — establish the policy and output contract.
- [RAG and tools](../../intermediate/10-evidence-grounded-prompting-and-rag-interfaces/README.md) — retrieval pipelines, tool selection, and evidence-aware answers.
- [Multimodal prompting](../../intermediate/12-multimodal-prompt-engineering/README.md) — visual/document context and grounding.
- [Prompt security](../../intermediate/13-prompt-security-and-untrusted-content/README.md) — injection, boundary, and validation controls.
- [Evaluation](../../../docs/07-evaluation.md) — measure retrieval, grounding, and end-to-end outcomes.
- [PromptOps](../../../docs/09-promptops.md) — versioning, tracing, and safe release practice.
...
**Current practice:**
1. **Explicit provenance and boundaries:** Tags or structured message fields make the intended role of each context item inspectable, but delimiters are not a security boundary.
2. **Selective long-context use:** Large context windows can reduce preprocessing for some tasks, but relevance, ordering, authorization, latency, and retrieval quality still need task-specific evaluation.
3. **Provider caching:** Some providers offer prompt or context caching with provider-specific eligibility, pricing, and latency behavior. Benchmark it with the exact model and traffic pattern rather than assuming instant reuse.
...
The [notebook](08_context_engineering.ipynb) keeps each experiment visible: it prints the rendered request, recorded response, parsed value, and deterministic assertion. Replay is offline by default; live mode is opt-in.
...
2. Change one prompt, policy, or budget variable in `lab08.py`; inspect the visible request, response, and deterministic gate.
