...
class TriageRecommendation(BaseModel):
    """A recommendation plus concise, externally checkable evidence."""

    evidence_checks: list[str] = Field(
        default_factory=list,
        description="Observable log facts or verification steps, not private chain-of-thought.",
    )
    recommended_action: str = Field(
        description="The exact action the on-call engineer should take."
    )
    self_reported_confidence: Literal["high", "medium", "low"] = Field(
        description="A qualitative model signal; never treated as authorization or probability."
    )
        ...
        requests.extend(
            [
                ...
                PromptRequest(
                    ...
                    system=(
                        "Return concise observable evidence checks and a recommendation. "
                        "Do not provide or request private chain-of-thought."
                    ),
                    messages=[Message(role="user", text=case["logs"])],
                    response_schema=TriageRecommendation,
                ),
                PromptRequest(
...
def classify_action_risk(action: str) -> Literal["safe", "requires_approval"]:
    if re.search(r"\b(scale|increase|pool|restart|database|drop|delete|rotate|compress)\b", action, re.I):
...
def majority_vote(actions: list[str]) -> tuple[str, float]:
    ...
    return (winners[0] if len(winners) == 1 else "no_consensus", top_count / len(actions))
...
def run_lab(client: ModelClient) -> dict[str, Metric | tuple[str, float] | str]:
    ...
    root_cause_metric = rate(
        "root_cause_named",
        sum(root_cause_named(result.evidence_checks, case["root_cause_keyword"]) for result, case in zip(cot, CASES)),
        len(CASES),
        "higher_is_better",
    )
    naive_root_cause = rate(
        "naive_root_cause_named",
        sum(root_cause_named(result.evidence_checks, case["root_cause_keyword"]) for result, case in zip(naive, CASES)),
        len(CASES),
        "higher_is_better",
    )
    decisions = [
    ...
    return {
        ...
        "recommended_action_exact": rate(
            "recommended_action_exact",
            sum(result.recommended_action == case["expected_action"] for result, case in zip(cot, CASES)),
            len(CASES),
            "higher_is_better",
        ),
        "routing_accuracy": rate(
            "routing_accuracy",
            sum(
                decision == case["expected_terminal_state"]
