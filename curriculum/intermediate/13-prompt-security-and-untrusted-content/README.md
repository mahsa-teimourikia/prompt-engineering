...
**Level:** Intermediate · **Estimated time:** 75–120 min · **Prerequisites:** Courses 01–12 and application security basics
...
A support ticket contains an indirect prompt injection; the application must separate data from instructions and authorize any consequential action.

## Core Concepts & Workflow

LLM applications often process trusted instructions and untrusted natural-language data in the same inference context. An attacker can exploit that ambiguity through prompt injection—for example, by placing “ignore previous instructions” inside a user message or retrieved document. Clear separation can improve behavior, but the application must assume that the model may still follow untrusted content.
...
A prompt-only rule such as “do not follow hostile instructions” is not a complete security control. Defense in depth combines provenance and boundaries with least-privilege tools, authorization in trusted code, validated outputs, monitoring, and human approval for consequential actions. Delimiters and schemas help structure the interface; they do not prove that an injection was neutralized.
...
Run the credential-free, self-contained [prompt-security notebook](../../intermediate/13-prompt-security-and-untrusted-content/13_prompt_security_and_untrusted_content.ipynb). Extend it with tenant isolation, proposal approval, and replay fixtures. Keep actions mocked: training must prove a block or approval state, never perform a live customer effect.
...
Continue with [Prompt evaluation](../../../docs/07-evaluation.md), [Agentic prompts](../../../docs/08-agentic-prompts.md), [PromptOps](../../../docs/09-promptops.md), [Technology review](../../../docs/10-technology-review.md), and [Agent identity and authorization](https://github.com/mahsa-teimourikia/agent-identity). Prompt security works when these controls form one system—not when one defensive sentence is added to a prompt.
...
**Current practice:**
1. **Preserve roles and provenance:** Use native message roles and label retrieved or user-controlled content as data. This improves structure but is not an injection-proof boundary.
2. **Validate typed proposals:** Schemas can reject malformed or disallowed output shapes. A schema-valid payload may still contain an unsafe intent, unauthorized resource, or false claim, so semantic and policy checks remain necessary.
3. **Layer controls:** Input classifiers, output checks, gateways, sandboxing, least-privilege tools, and human approval can reduce risk when each has a tested responsibility. No gateway or guardrail detects every attack.

## Lab walkthrough

The [notebook](13_prompt_security_and_untrusted_content.ipynb) demonstrates a prompt injection attempt against a vulnerable request. It then layers explicit provenance, a typed proposal, and an application-side egress policy. The deterministic gate blocks the fixture; the exercise does not claim that delimiters or one test neutralize prompt injection in general.

The [notebook](13_prompt_security_and_untrusted_content.ipynb) keeps each experiment visible: it prints the rendered request, recorded response, parsed value, and deterministic assertion. Replay is offline by default; live mode is opt-in.
...
2. Change one prompt, policy, or budget variable in `lab13.py`; inspect the visible request, response, and deterministic gate.
...
- https://docs.nvidia.com/nemo/guardrails/configure-guardrails/guardrail-catalog
- https://genai.owasp.org/llm-top-10/
- https://github.com/mahsa-teimourikia/agent-identity
- https://openai.github.io/openai-agents-python/guardrails/
- https://opentelemetry.io/docs/concepts/semantic-conventions/
- https://www.nist.gov/artificial-intelligence/ai-research-security-and-resilience
