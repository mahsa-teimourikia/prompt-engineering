...
The existing self-contained [evaluation notebook](../curriculum/advanced/14-prompt-evaluation/14_prompt_evaluation.ipynb) provides a credential-free starting point. Extend it with trajectory fields such as `method`, `model_calls`, `tool_calls`, `verification_result`, `latency_ms`, and `estimated_cost`.
