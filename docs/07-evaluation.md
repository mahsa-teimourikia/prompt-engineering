...
Run the credential-free, self-contained [evaluation notebook](../curriculum/advanced/14-prompt-evaluation/14_prompt_evaluation.ipynb). Add your six cases, make a candidate exceed the cost budget, and confirm the gate blocks it. Then add one redacted production-inspired regression fixture and document its source, risk, and expected behavior.
