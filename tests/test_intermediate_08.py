...
def test_course_08_policy_boundary_and_budget():
    ...
    assert results["routing_accuracy"].numerator == 3
    assert results["routing_accuracy"].denominator == 3
    ...
    assert lab08.decide("YES", 7) == "rejected"
    assert lab08.decide("NO", 3) == "rejected"
