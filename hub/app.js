...
function renderWorkspace() {
  ...
  const body = tab === "learn" ? `<p class="outcome">${selected.outcome}</p><p>Read the in-depth theory, trade-offs, failure modes, and cited sources before opening the executable notebook.</p><a class="button" href="${link(selected.material)}" target="_blank">Read lesson ↗</a>` : tab === "lab" ? `<p class="outcome">The notebook is the primary learning artifact: theory, deterministic code, an experiment, and reflection live together.</p><p>Run the companion Python lab locally with <code>python3 ${selected.lab}</code>. It uses no API key or external side effects.</p><a class="button" href="${link(selected.notebook)}" target="_blank">Open self-contained notebook ↗</a> <a class="source" href="${link(selected.lab)}" target="_blank">View reusable lab code ↗</a>` : `<p class="outcome">${check.question}</p><div class="choices">${check.choices.map((choice, index) => `<button data-choice="${index}">${choice}</button>`).join("")}</div><p id="feedback" class="feedback"></p><a href="quiz/">Take the full quiz ↗</a>`;
